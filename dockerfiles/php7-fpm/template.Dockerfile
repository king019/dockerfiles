...
RUN mkdir /usr/log && mkdir /run/php \
    ...
    && apt-get -qq update \
    && sh -c "apt-cache search php7.4 | cut -d' ' -f1 | egrep -v \"(apache|litespeed|gmagick|libsodium|yac)\" | xargs apt-get -qq install -y --no-install-recommends" \
    ...
    && sh -c "apt-cache search php7.3 | cut -d' ' -f1 | egrep -v \"(apache|litespeed|gmagick|libsodium|yac)\" | xargs apt-get -qq install -y --no-install-recommends" \
